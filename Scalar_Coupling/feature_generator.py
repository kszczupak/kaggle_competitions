...
def get_train_and_test_sets():
    ...
    print("-> Batches creation complete")
    
    with mp.Pool(mp.cpu_count()) as p:
        results = p.map(create_features, all_batches)
    
    print("-> Results from parallel pool completed")
...
#     data_with_atoms_info = add_atoms_info(data)
    ...
    data_with_features = add_distances(data)
...
train_df = add_atoms_info(train_df)
test_df = add_atoms_info(test_df)
main()